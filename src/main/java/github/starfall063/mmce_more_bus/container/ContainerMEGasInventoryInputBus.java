...
public final class ContainerMEGasInventoryInputBus extends AEBaseContainer {
    ...
    public ContainerMEGasInventoryInputBus(InventoryPlayer inventoryPlayer, MEGasInventoryInputBus bus) {
        super(inventoryPlayer, bus);
        this.bus = bus;
        IItemHandlerModifiable markerHandler = new MarkerItemHandler(bus);
        ItemStackHandler previewSlots = new ItemStackHandler(MEGasInventoryInputBus.SLOT_COUNT);
    ...
    public static int markerSlotCount() {
        return MEGasInventoryInputBus.SLOT_COUNT;
    }
    ...
    private static final class MarkerItemHandler implements IItemHandlerModifiable {
        ...
        private MarkerItemHandler(MEGasInventoryInputBus bus) {
            this.bus = bus;
        }

        private static GasStack getGas(ItemStack stack) {
            if (stack.isEmpty() || !(stack.getItem() instanceof IGasItem gasItem)) return null;
            return gasItem.getGas(stack);
        }
        ...
        public void setStackInSlot(int slot, ItemStack stack) {
            ...
            if (gas == null) bus.clearMarker(slot);
            else bus.setMarker(slot, gas);
        }
